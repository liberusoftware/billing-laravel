...
final class MeterResource extends Resource
{
    ...
    public static function form(Schema $schema): Schema { return $schema->components([TextInput::make('name')->required(), TextInput::make('code')->required(), TextInput::make('unit')->required(), TextInput::make('unit_price_minor')->numeric()->required(), TextInput::make('currency')->length(3)->required(), TextInput::make('threshold')->numeric(), Toggle::make('active')]); }
    public static function table(Table $table): Table { return $table->columns([TextColumn::make('name')->searchable(), TextColumn::make('code')->sortable(), TextColumn::make('unit'), TextColumn::make('unit_price_minor'), TextColumn::make('currency'), TextColumn::make('active')->boolean()])->defaultSort('id', 'desc'); }
    public static function getPages(): array { return ['index' => ListMeters::route('/'), 'create' => CreateMeter::route('/create')]; }
}
