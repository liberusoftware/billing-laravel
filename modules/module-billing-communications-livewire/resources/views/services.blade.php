<div>
    ...
    <ul wire:loading.class="opacity-50">
        ...
        @forelse ($services as $service)
            <li wire:key="communication-service-{{ $service->id }}">{{ $service->name }} ({{ $service->status }}) <button type="button" wire:click="$set('selectedServiceId', {{ $service->id }})">{{ __('Select') }}</button> <button type="button" wire:click="transition">{{ __('Update status') }}</button></li>
        @empty
