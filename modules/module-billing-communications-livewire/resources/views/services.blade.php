<div>
    ...
    @if (session('billing-communications-services-message'))<p role="status">{{ session('billing-communications-services-message') }}</p>@endif
    ...
    <ul wire:loading.class="opacity-50">
        ...
        @forelse ($services as $service)
            <li wire:key="communication-service-{{ $service->id }}">{{ $service->name }} ({{ $service->status }}) <button type="button" wire:click="$set('selectedServiceId', {{ $service->id }})">{{ __('Select') }}</button> <button type="button" wire:click="transitionService">{{ __('Update status') }}</button></li>
        @empty
            <li>{{ __('No communication services found.') }}</li>
        @endforelse
    </ul>
</div>
