<div>
    ...
    <ul wire:loading.class="opacity-50"><li wire:loading>{{ __('Loading…') }}</li>@forelse ($records as $record)<li wire:key="catalog-record-{{ $record->getKey() }}">{{ $record->name }} ({{ $record->code }}) — {{ $record->status->value }} <button type="button" wire:click="transition({{ $record->getKey() }})">{{ __('Update lifecycle') }}</button></li>@empty<li>{{ __('No records found.') }}</li>@endforelse</ul>
</div>
